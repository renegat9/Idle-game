...
#
# Usage :
#   bash generate_images.sh                   # tout générer (loot + héros + zones + monstres + boss + musique)
...
#   bash generate_images.sh --bosses          # portraits boss mondiaux uniquement
...
# ── Parse options ─────────────────────────────────────────────────────────────
DO_LOOT=false DO_HEROES=false DO_ZONES=false DO_MONSTERS=false DO_ELITE_ONLY=false DO_BOSSES=false DO_MUSIC=false
...
if ! $DO_LOOT && ! $DO_HEROES && ! $DO_ZONES && ! $DO_MONSTERS && ! $DO_BOSSES && ! $DO_MUSIC; then
    DO_LOOT=true; DO_HEROES=true; DO_ZONES=true; DO_MONSTERS=true; DO_BOSSES=true; DO_MUSIC=true
fi
...
# ── Boss mondiaux ─────────────────────────────────────────────────────────────
if $DO_BOSSES; then
    echo -e "\n${YELLOW}══ PORTRAITS BOSS MONDIAUX ══${NC}"
    $PHP "$ARTISAN" images:bosses $FORCE $DELAY $LIST_FLAG || true
fi
