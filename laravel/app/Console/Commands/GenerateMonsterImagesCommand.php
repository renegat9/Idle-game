...
class GenerateMonsterImagesCommand extends Command
{
    ...
    public function handle(GeminiService $gemini): int
    {
        ...
        foreach ($monsters as $i => $monster) {
            ...
            $needsBase  = !$eliteOnly
                && ($all || $force || empty($monster->image_path) || str_starts_with($monster->image_path ?? '', 'images/placeholders/'));

            $needsElite = ($eliteOnly || $all)
                && ($force || empty($monster->elite_image_path) || str_starts_with($monster->elite_image_path ?? '', 'images/placeholders/'));
